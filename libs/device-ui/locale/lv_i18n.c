...
static const lv_i18n_phrase_t zh_cn_singulars[] = {
    ...
    {"no SD card detected", "未检测到 SD 卡"},
    {"map tiles not found!", "未找到地图瓦片！"},
    {"Backup & Restore", "备份与恢复"},
    {"Client\nClient Mute\nRouter\nRepeater\nTracker\nSensor\nTAK\nClient Hidden\nLost & Found\nTAK Tracker", "客户端\n客户端静默\n路由器\n中继\n追踪器\n传感器\nTAK\n隐藏客户端\n失物招领\nTAK 追踪器"},
    {"Backup", "备份"},
    {"Restore", "恢复"},
    {"Public/Private Key", "公私钥"},
    {"NodeDB Reset\nFactory Reset\nClear Chat History", "重置节点库\n恢复出厂设置\n清空聊天记录"},
    {"Channel", "信道"},
    {"New Message from\n", "新消息来自\n"},
    ...
    {"No map tiles found on SDCard!", "SD 卡上未找到地图瓦片！"},
    {"Language: %s", "语言：%s"},
    {"Region: %s", "区域：%s"},
    {"User name: %s", "用户名：%s"},
    ...
    {"%s: %d GB (%s)\nUsed: %0.2f GB (%d%%)", "%s: %d GB (%s)\n已用: %0.2f GB (%d%%)"},
    {"SD slot empty", "SD 卡槽为空"},
    {"SD invalid format", "SD 卡格式无效"},
    {"SD mbr not found", "未找到 SD 卡引导记录"},
    {"SD card error", "SD 卡错误"},
    {"SD unknown error", "SD 卡未知错误"},
    {"About / Legal", "关于 / 法律信息"},
    {"About & Attribution", "关于与致谢"},
...
static const lv_i18n_phrase_t zh_tw_singulars[] = {
    {"Reboot into BaseUI?", "重新開機進入 BaseUI？"},
    {"no new messages", "沒有新訊息"},
    {"1 of 1 nodes online", "1/1 個節點在線"},
    {"uptime 00:00:00", "運作時間：00:00:00"},
    {"LoRa 0.0 MHz", "LoRa 0.0 MHz"},
    {"no signal", "沒有訊號"},
    {"silent", "靜音"},
    {"no SD card detected", "未偵測到 SD 卡"},
    {"Heap: 0\nLVGL: 0", "記憶體：0\nLVGL：0"},
    {"DEL", "刪除"},
    {"map tiles not found!", "找不到地圖圖磚！"},
    {"Settings", "設定"},
    {"User name: ", "使用者名稱："},
    {"Modem Preset: LONG FAST", "調變預設：長距離快速"},
    {"Channel: LongFast", "頻道：長距離快速"},
    {"Role: Client", "角色：用戶端"},
    {"WiFi: <not setup>", "WiFi：<未設定>"},
    {"Screen Timeout: 60s", "螢幕逾時：60 秒"},
    {"Lock: off/off", "鎖定：關/關"},
    {"Screen Brightness: 60%", "螢幕亮度：60%"},
    {"Theme: Dark", "主題：深色"},
    {"Screen Calibration: default", "螢幕校正：預設"},
    {"Input Control: none/none", "輸入控制：無/無"},
    {"Message Alert Buzzer: on", "訊息提示音：開"},
    {"Language: English", "語言：繁體中文"},
    {"Configuration Reset", "重設設定"},
    {"Backup & Restore", "備份與還原"},
    {"Reboot / Shutdown", "重新開機 / 關機"},
    {"Tools", "工具"},
    {"Mesh Detector", "Mesh 偵測器"},
    {"Signal Scanner", "訊號掃描"},
    {"Trace Route", "路由追蹤"},
    {"Neighbors", "鄰近節點"},
    {"Statistics", "統計"},
    ...
    {"Group Channels", "群組頻道"},
    {"no messages", "沒有訊息"},
    {"Settings & Tools", "設定與工具"},
    {"Settings (advanced)", "設定（進階）"},
    {"Locations Map", "位置地圖"},
    {"no chats", "沒有聊天"},
    {"Node Search", "節點搜尋"},
    {"Packet Statistics", "封包統計"},
    {"Node Options", "節點選項"},
    {"LoRa TX off!", "LoRa 發射已關閉！"},
    {"Short Name", "簡稱"},
    {"Long Name", "全名"},
    {"Primary Channel", "主要頻道"},
    {"<unset>", "<未設定>"},
    {"Secondary Channels", "次要頻道"},
    {"LONG FAST\nLONG SLOW\n-- deprecated --\nMEDIUM SLOW\nMEDIUM FAST\nSHORT SLOW\nSHORT FAST\nLONG MODERATE\nSHORT TURBO", "長距離快速\n長距離慢速\n-- 已棄用 --\n中距離慢速\n中距離快速\n短距離慢速\n短距離快速\n長距離適中\n短距離極速"},
    {"Client\nClient Mute\nRouter\nRepeater\nTracker\nSensor\nTAK\nClient Hidden\nLost & Found\nTAK Tracker", "用戶端\n靜音用戶端\n路由器\n中繼器\n追蹤器\n感測器\nTAK\n隱藏用戶端\n失物招領\nTAK 追蹤器"},
    {"WiFi SSID", "WiFi 名稱"},
    {"WiFi pre-shared Key", "WiFi 預共享金鑰"},
    {"Brightness: 60%", "亮度：60%"},
    {"Dark\nLight", "深色\n淺色"},
    {"Timeout: 60s", "逾時：60 秒"},
    {"Screen Lock", "螢幕鎖定"},
    ...
    {"Backup", "備份"},
    {"Restore", "還原"},
    {"Public/Private Key", "公鑰/私鑰"},
    {"NodeDB Reset\nFactory Reset\nClear Chat History", "重設節點資料庫\n恢復原廠設定\n清除聊天記錄"},
    {"Channel Name", "頻道名稱"},
    ...
    {"Start", "開始"},
    {"choose\nnode", "選擇\n節點"},
    {"choose target node", "選擇目標節點"},
    {"New Message from\n", "新訊息來自\n"},
    {"Restoring messages ...", "正在還原訊息..."},
    {"Please set region and name", "請設定區域與名稱"},
    {"Region", "區域"},
    {"Resync ...", "正在重新同步..."},
    {"OK", "確定"},
    {"Cancel", "取消"},
    ...
    {"on", "開啟"},
    {"off", "關閉"},
    {"Screen Calibration: %s", "螢幕校正：%s"},
    {"done", "完成"},
    {"default", "預設"},
    {"Client\nClient Mute\nTracker\nSensor\nTAK\nClient Hidden\nLost & Found\nTAK Tracker", "用戶端\n靜音用戶端\n追蹤器\n感測器\nTAK\n隱藏用戶端\n失物招領\nTAK 追蹤器"},
    {"Rebooting ...", "正在重新開機..."},
    {">> Programming mode <<", ">> 燒錄模式 <<"},
    {"Enter Text ...", "輸入文字..."},
    {"!Enter Filter ...", "!輸入篩選條件..."},
    {"Enter Filter ...", "輸入篩選條件..."},
    {"FrequencySlot: %d (%g MHz)", "頻率槽：%d (%g MHz)"},
    {"Brightness: %d%%", "亮度：%d%%"},
    {"Timeout: off", "逾時：關閉"},
    {"Timeout: %ds", "逾時：%d 秒"},
    {"No map tiles found on SDCard!", "SD 卡上找不到地圖圖磚！"},
    {"Locations Map (%d/%d)", "位置地圖 %d/%d"},
    {"Stop", "停止"},
    {"heard: !%08x", "已收到：!%08x"},
    {"Packet Log: %d", "封包記錄：%d"},
    {"Language: %s", "語言：%s"},
    {"Screen Timeout: off", "螢幕逾時：關閉"},
    {"Screen Timeout: %ds", "螢幕逾時：%d 秒"},
    {"Screen Brightness: %d%%", "螢幕亮度：%d%%"},
    {"Theme: %s", "主題：%s"},
    {"Region: %s", "區域：%s"},
    {"User name: %s", "使用者名稱：%s"},
    {"Device Role: %s", "裝置角色：%s"},
    {"Modem Preset: %s", "調變預設：%s"},
    {"WiFi: %s", "WiFi：%s"},
    {"<not set>", "<未設定>"},
    {"Util %0.1f%%  Air %0.1f%%", "使用率 %0.1f%%　空中使用率 %0.1f%%"},
    {"hops: %d", "跳數：%d"},
    {"unknown", "未知"},
    {"Connected!", "已連線！"},
    {"Disconnected!", "已中斷連線！"},
    ...
    {"Banner only", "僅橫幅"},
    {"Sound only", "僅音效"},
    {"Message Alert: %s", "訊息提示：%s"},
    {"Channel: %s", "頻道：%s"},
    {"Failed to write keys!", "寫入金鑰失敗！"},
    {"Failed to restore keys!", "還原金鑰失敗！"},
    {"Failed to parse keys!", "解析金鑰失敗！"},
    {"Failed to retrieve keys!", "取得金鑰失敗！"},
    {"%d active chat(s)", "%d 個進行中的聊天"},
    {"New message from \n%s", "新訊息來自\n%s"},
    {"Input Control: %s/%s", "輸入控制：%s/%s"},
    {"%d of %d nodes online", "%d/%d 個節點在線"},
    {"Filter: %d of %d nodes", "篩選：%d/%d 個節點"},
    {"%d new message", "%d 則新訊息"},
    {"%d new messages", "%d 則新訊息"},
    {"uptime: %02d:%02d:%02d", "運作時間：%02d:%02d:%02d"},
    {"%s: %d GB (%s)\nUsed: %0.2f GB (%d%%)", "%s：%d GB (%s)\n已用：%0.2f GB (%d%%)"},
    {"SD slot empty", "SD 卡槽是空的"},
    {"SD invalid format", "SD 卡格式無效"},
    {"SD mbr not found", "找不到 SD 卡開機記錄"},
    {"SD card error", "SD 卡錯誤"},
    {"SD unknown error", "SD 卡未知錯誤"},
    {"Heap: %d (%d%%)\nLVGL: %d (%d%%)", "記憶體：%d (%d%%)\nLVGL：%d (%d%%)"},
    {"Used: ...", "已用：..."},
    {NULL, NULL} // End mark
};
...
static uint8_t zh_tw_plural_fn(int32_t num)
{
...
static const lv_i18n_lang_t zh_tw_lang = {
    .locale_name = "zh-TW",
    .singulars = zh_tw_singulars,

    .locale_plural_fn = zh_tw_plural_fn
};

const lv_i18n_language_pack_t lv_i18n_language_pack[] = {
    &en_lang, &bg_lang, &cs_lang, &da_lang, &de_lang, &el_lang, &es_lang, &fi_lang, &fr_lang, &hu_lang, &it_lang,    &nl_lang,
    &no_lang, &pl_lang, &pt_lang, &ro_lang, &ru_lang, &se_lang, &sl_lang, &sr_lang, &tr_lang, &uk_lang, &zh_cn_lang, &zh_tw_lang,
    NULL // End mark
};
