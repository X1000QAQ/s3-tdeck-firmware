...
class ISdCard
{
  public:
    ...
    // 某些卡（exFAT 常见）SdFat 无法统计剩余簇，此时 usedBytes()/freeBytes() 不可信
    static void markUptime(void); // 诊断：记录运行时长，供下次启动判断"上次活了多久"
...
class SdFsCard : public ISdCard
{
  ...
  private:
    ...
    bool statsOk = false;
    uint64_t freeBytesCached = 0;
    uint32_t lastAttemptMs = 0; // 失败后限流重试
};
