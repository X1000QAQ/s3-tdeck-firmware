...
#include "Arduino.h"
#include "graphics/map/CoordTransform.h"
...
class GeoPoint
{
  public:
    ...
    void setZoom(uint8_t zoom)
    {
        ...
        // NOTE: for GCJ-02 tile sets (高德/Gaode ...) the position must be shifted first,
        // otherwise everything is drawn with a ~500m offset. latitude/longitude stay WGS-84.
        FLOATING_POINT mLat = latitude;
        FLOATING_POINT mLon = longitude;
        if (MapTileSettings::getGcjOffset()) {
            double dLat = latitude, dLon = longitude;
            CoordTransform::wgs2gcj(dLat, dLon);
            mLat = dLat;
            mLon = dLon;
        }
        auto n = 1 << zoom;
        auto size = MapTileSettings::getTileSize();
        auto lat_rad = mLat * FLOATING_POINT(PI) / FLOATING_POINT(180.0);
        auto xRaw = (mLon + 180.0) / 360.0 * n;
    ...
    void move(int16_t scrollX, int16_t scrollY)
    {
        ...
        // the tile grid is GCJ-02; convert back so that latitude/longitude stay WGS-84
        if (MapTileSettings::getGcjOffset()) {
            double dLat = latitude, dLon = longitude;
            CoordTransform::gcj2wgs(dLat, dLon);
            latitude = dLat;
            longitude = dLon;
        }
    }
