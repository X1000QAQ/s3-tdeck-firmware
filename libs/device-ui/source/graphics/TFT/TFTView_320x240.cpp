...
#include "input/PinyinDictZh.h"
#include "lv_i18n.h"
#include <Wire.h>
#if defined(TOUCH_DIAG)
// 探测 GT911 是否在指定地址应答（读产品 ID 寄存器 0x8140 的第一个字节）
static bool diagGt911(uint8_t addr)
{
    ...
    if (Wire.endTransmission(false) != 0)
        return false;
    if (Wire.requestFrom((int)addr, 4) != 4)
        return false;
    for (int i = 0; i < 4; i++)
        (void)Wire.read();
    return true;
...
#define PYIME_CAND_H 34 // V42: 拼音候选栏高度(px)，固定成一行候选字
// 拼音候选词栏：始终贴键盘正上方，并沿用键盘按键配色（浅色/深色主题都适配）。
// LVGL 只在 lv_ime_pinyin_set_keyboard() 里对齐一次，而本 UI 会按输入框位置移动键盘，
// 因此每次显示键盘（消息、WiFi 密码、频道名……所有输入界面）都要重新对齐一次。
static void alignPinyinCandPanel(lv_obj_t *ime, lv_obj_t *kb)
{
    if (!ime || !kb)
        return;
    ...
    if (!cand)
        return;
    lv_obj_set_size(cand, LV_PCT(100), PYIME_CAND_H); // V42: 固定成一行高度

    // V42: 键盘贴屏幕顶时上方放不下候选栏 ⇒ 把键盘整体下移一行给它腾地方，
    // 而不是把候选栏翻到键盘下方（那样会盖住输入框）。
    lv_area_t kb_area;
    lv_obj_update_layout(kb);
    lv_obj_get_coords(kb, &kb_area);
    if (kb_area.y1 < PYIME_CAND_H)
        lv_obj_set_y(kb, PYIME_CAND_H);

    lv_obj_update_layout(kb);
    lv_obj_align_to(cand, kb, LV_ALIGN_OUT_TOP_MID, 0, 0);
    if (lv_obj_get_y(cand) < 0) // 兜底：候选栏绝不出屏
        lv_obj_set_y(cand, 0);
    lv_obj_move_foreground(cand); // 键盘会 move_foreground，候选栏必须跟着到最前
    lv_color_t key_bg = lv_obj_get_style_bg_color(kb, LV_PART_ITEMS);
    lv_color_t key_txt = lv_obj_get_style_text_color(kb, LV_PART_ITEMS);
    lv_obj_set_style_bg_opa(cand, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(cand, key_bg, 0);
    lv_obj_set_style_text_color(cand, key_txt, 0);
    lv_obj_set_style_text_color(cand, key_txt, LV_PART_ITEMS);
    lv_obj_set_style_bg_opa(cand, LV_OPA_40, LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_bg_color(cand, key_txt, LV_PART_ITEMS | LV_STATE_CHECKED);
}
#endif

#if LV_USE_IME_PINYIN
// 候选词栏跟随键盘动画：键盘是【滑上来】的，只在 showKeyboard() 里对齐一次会留在原位，
// 表现为"候选栏跑到键盘下面"。这里放一个正在滑动、需要跟随的键盘指针，供动画回调每帧使用。
static lv_obj_t *ime_pinyin = nullptr;   // 定义提前（原在下方），供动画回调引用
static lv_obj_t *candTrackKb = nullptr;  // 非空时：该键盘滑动期间，候选栏每帧跟着重新贴
#endif
...
#if LV_USE_IME_PINYIN
    // 键盘滑动时（开/关都走这里）每帧把候选栏重新贴到键盘正上方，
    // 否则它会留在动画开始时的位置 —— 表现为"候选栏跑到键盘下面"。
    if (candTrackKb && (lv_obj_t *)var == candTrackKb)
        alignPinyinCandPanel(ime_pinyin, candTrackKb);
#endif
}
...
#if LV_USE_IME_PINYIN
// 中文拼音输入法实例（LVGL 内置 IME）
// static lv_obj_t *ime_pinyin = nullptr;  // 已提前到文件顶部定义
#endif

void TFTView_320x240::updatePinyinDict(void)
{
#if LV_USE_IME_PINYIN
    if (!ime_pinyin)
        return;
    bool tw = (db.uiConfig.language == meshtastic_Language_TRADITIONAL_CHINESE);
    lv_ime_pinyin_set_dict(ime_pinyin, (lv_pinyin_dict_t *)lv_pinyin_dict_zh); // 2026-09-19 繁体词库已移除，统一用简体
    ILOG_DEBUG("pinyin dict: %s", tw ? "traditional" : "simplified");
#endif
}
...
#if LV_USE_IME_PINYIN
    // 中文拼音输入法：LVGL 内置候选词 IME，绑定到虚拟键盘
    if (objects.keyboard && ime_pinyin == nullptr) {
        ime_pinyin = lv_ime_pinyin_create(lv_obj_get_parent(objects.keyboard));
        lv_ime_pinyin_set_keyboard(ime_pinyin, objects.keyboard);
        lv_ime_pinyin_set_mode(ime_pinyin, LV_IME_PINYIN_MODE_K26);
        // 候选词栏：LVGL 默认把它放在父对象(屏幕)顶部且文字色未设置(深色主题下黑字看不见)
        // 这里显式贴到键盘正上方，并沿用键盘自己的配色（浅色/深色主题都一致）
        alignPinyinCandPanel(ime_pinyin, objects.keyboard);
        // 用自带词典替换 LVGL 默认词典（默认是繁体小词典，很多字不在字库里会显示空白）
        updatePinyinDict();
    }
#endif
...
void TFTView_320x240::ui_event_navHome(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_CLICKED) {
        ...
        // v64: 写入前先校验 —— 非法就不落盘（避免污染 NVS / 造成永久全灰 ✗）
        if (!MapTileSettings::isValidLatLon(lat, lon)) {
            ILOG_WARN("v64: refuse to save invalid home (lat=%f lon=%f)", lat, lon);
            return;
        }
    ...
    if (!map) {
        ...
        if (hasPosition) {
            if (db.uiConfig.map_data.has_home) {
                // v64: home 经统一校验后才使用（非法 ⇒ 回退默认中心 ✓）
                float hLat = db.uiConfig.map_data.home.latitude * 1e-7f;
                float hLon = db.uiConfig.map_data.home.longitude * 1e-7f;
                uint8_t hZoom = (uint8_t)db.uiConfig.map_data.home.zoom;
                bool hOk = MapTileSettings::sanitize(hLat, hLon, hZoom);
                map->setHomeLocation(hLat, hLon);
                map->setZoom(hZoom);
                ILOG_INFO("v64 map center: lat=%f lon=%f zoom=%u src=home(%s)", hLat, hLon, hZoom, hOk ? "ok" : "fallback");
            } else {
                // v64: 实时位置同样过校验；zoom 用 zoomDefault（不再硬编码 13 ✗）
                float pLat = myLatitude * 1e-7f, pLon = myLongitude * 1e-7f;
                uint8_t pZoom = MapTileSettings::getDefaultZoom();
                bool pOk = MapTileSettings::sanitize(pLat, pLon, pZoom);
                map->setHomeLocation(pLat, pLon);
                map->setZoom(pZoom);
                ILOG_INFO("v64 map center: lat=%f lon=%f zoom=%u src=gps(%s)", pLat, pLon, pZoom, pOk ? "ok" : "fallback");
            }
            ...
            ILOG_INFO("v64 map center: lat=%f lon=%f zoom=%u src=default(fallback)",
                      MapTileSettings::getDefaultLat(), MapTileSettings::getDefaultLon(),
                      MapTileSettings::getDefaultZoom());
        }
...
{
    int32_t val[] = {0, -1, -1, 4, 3, 6, 5, 1, 7, 2, 8, 9, 10, 11};
...
meshtastic_Config_LoRaConfig_ModemPreset TFTView_320x240::val2preset(uint32_t val)
{
    meshtastic_Config_LoRaConfig_ModemPreset preset[] = {
        ...
        meshtastic_Config_LoRaConfig_ModemPreset_SHORT_SLOW,  meshtastic_Config_LoRaConfig_ModemPreset_SHORT_TURBO,
...
void TFTView_320x240::setLocale(meshtastic_Language lang)
{
    ...
    updatePinyinDict(); // 简/繁切换时同步拼音候选词库
...
void TFTView_320x240::showKeyboard(lv_obj_t *textArea)
{
    ...
    uint32_t v = lv_display_get_vertical_resolution(displaydriver->getDisplay());
#if LV_USE_IME_PINYIN
    // V45: 消息输入框分支也必须给候选栏留一行，否则键盘滑上来后候选栏正好压住输入框
    const int32_t cand_h = (ime_pinyin != nullptr) ? PYIME_CAND_H : 0;
#else
    const int32_t cand_h = 0;
#endif
    ...
    if (textArea == objects.message_input_area) {
        ...
        // V45: 面板上移 kb_h + cand_h ⇒ 输入框落在候选栏之上，候选栏夹在输入框与键盘之间
        lv_anim_set_values(&a1, panelY, kbdPanelBaseY - (int32_t)(kb_h + cand_h));
        ...
        if (text_coords.y1 > kb_h + 30 + (int32_t)cand_h) {
            // if enough place above put under top panel
            lv_obj_set_pos(objects.keyboard, 0, 28 + cand_h);
        } else if ((text_coords.y1 + 10) > v / 2) {
            // if text area is at lower half then place above text area
            int32_t kb_y = text_coords.y1 - kb_h - 2 - cand_h;
            if (kb_y < 0)
                kb_y = 0;
            lv_obj_set_pos(objects.keyboard, 0, kb_y);
        } else {
            // place below text area
            int32_t kb_y = text_coords.y2 + 3 + cand_h;
            if (cand_h > 0 && kb_y + (int32_t)kb_h > (int32_t)v) {
                int32_t bottom = (int32_t)v - (int32_t)kb_h;
                kb_y = (bottom > (int32_t)text_coords.y2 + 3) ? bottom : (int32_t)text_coords.y2 + 3;
            }
            lv_obj_set_pos(objects.keyboard, 0, kb_y);
        }
    }
...
#if LV_USE_IME_PINYIN
    candTrackKb = objects.keyboard; // 登记：此后键盘滑动（含动画）时，候选栏每帧跟随
    alignPinyinCandPanel(ime_pinyin, objects.keyboard); // 键盘位置随输入框变化，这里先贴一次
#endif
}
...
/**
 * 格式化并显示 SD 卡信息（不重新初始化卡）。统计值可能稍后才可用，故单独成函数以便刷新。
 */
void TFTView_320x240::updateSDCardInfo(void)
{
    ...
    const char *cardStr = cardType == ISdCard::eMMC    ? "MMC"
                          : cardType == ISdCard::eSD   ? "SDSC"
                          : cardType == ISdCard::eSDHC ? "SDHC"
                          : cardType == ISdCard::eSDXC ? "SDXC"
                                                       : "UNKN";
    const char *fatStr = fatType == ISdCard::eExFat   ? "exFAT"
    ...
    // 只显示容量与格式。统计剩余空间要遍历整张 FAT，在"显示/SD 共用 SPI"的板子上会阻塞数秒，
    // 期间屏幕会撕裂成乱码（用户实测），因此不再在 UI 线程里触发扫描。
    (void)fatStr;
    sprintf(buf, "%s: %d GB (%s)", cardStr, totalSpaceGB, fatStr);
    ...
    if (screensInitialised) {
        ...
        if (curtime - lastrun1 >= 1) { // call every 1s
            ...
            if (curtime - lastrun5 >= 5) { // call every 5s
                lastrun5 = curtime;
#if defined(TOUCH_DIAG)
                static bool diagShown = false;
                if (!diagShown) {
                    diagShown = true;
                    // 主总线扫描结果（短，保证弹窗能完整显示）
                    static char shortMsg[64];
                    static char fullMsg[400];
                    int sn = snprintf(shortMsg, sizeof(shortMsg), "I2C 18/8:"), fn = 0;
                    fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, "SDA18/SCL8:");
                    Wire.end();
                    Wire.begin(18, 8);
                    ...
                    for (uint8_t a = 0x08; a < 0x78; a++) {
                        ...
                        if (Wire.endTransmission() == 0) {
                            fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, " %02X", a);
                            sn += snprintf(shortMsg + sn, sizeof(shortMsg) - sn, " %02X", a);
                            found++;
                        }
                    }
                    if (!found) {
                        sn += snprintf(shortMsg + sn, sizeof(shortMsg) - sn, " none");
                        fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, " none");
                    }
                    // GT911 产品 ID 探测
                    fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, "\nGT911 5D:%s 14:%s",
                                   diagGt911(0x5D) ? "yes" : "no", diagGt911(0x14) ? "yes" : "no");
                    // 交换引脚再扫一遍（仅写入文件，不占弹窗）
                    Wire.end();
                    Wire.begin(8, 18);
                    fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, "\nSDA8/SCL18:");
                    ...
                    Wire.end();
                    Wire.begin(18, 8);
                    ILOG_ERROR("[DIAG] %s", fullMsg);
                    // 结果同时写 SD 卡根目录 diag.txt（插卡时可离线查看）
#if defined(HAS_SDCARD)
                    {
                        FsFile f = SDFs.open("/diag.txt", O_WRONLY | O_CREAT | O_TRUNC);
                        if (f) {
                            f.println(fullMsg);
                            f.close();
                            fn += snprintf(fullMsg + fn, sizeof(fullMsg) - fn, " -> /diag.txt");
                        }
                    }
#endif
                    messageAlert(shortMsg, true);
                }
#endif
            ...
            if (curtime - lastrun10 >= 10) { // call every 10s
                lastrun10 = curtime;
                updateFreeMem();
#ifdef ENABLE_SD_DIAG
                if (sdCard)
                    SdFsCard::markUptime(); // 诊断打点
#endif
