...
#if defined(T_DECK) && !defined(TDECK_NO_I2C_KEYBOARD)
    // 克隆板注意：本板无 T-Deck 官方键盘（输入是摇杆 encoder + 触摸）。
    // 若无条件创建 TDeckKeyboardInputDriver(0x55)，驱动会持续轮询不存在的键盘，
    // 造成 ~20Hz I²C 超时错误刷屏（实测 addr=0x55）。定义 TDECK_NO_I2C_KEYBOARD 可关闭。
